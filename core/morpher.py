...
def get_auto_facearae(points, src_img):
    listx = []
    listy = []
    x_limit = src_img.shape[1]
    y_limit = src_img.shape[0]
    for n in points:
        listx.append(n[0])
        listy.append(n[1])
    new_point = (int(min(listx)*0.8), int(min(listy)*0.8), int(max(listx)+(x_limit-max(listx))*0.2)-int(min(listx)*0.8), int(max(listy)+(y_limit-max(listy))*0.2)-int(min(listy)*0.8))
    return new_point
